...
КРИТИЧЕСКИ ВАЖНО — СТАРТ ИГРЫ:
- Экран старта ОБЯЗАН реагировать на КЛИК/ТАЧ по canvas!
- canvas.addEventListener("click") — запускает игру из старта
- canvas.addEventListener("touchstart") — для мобильных
- НЕ только клавиатура! Многие пользователи тыкают мышкой.
- Инструкция на стартовом экране: "Нажми или кликни чтобы играть"
- Пример: canvas.addEventListener("click", () => {
    if (gameState === "start") startGame();
  });

КРИТИЧЕСКИ ВАЖНО — ИНИЦИАЛИЗАЦИЯ ПЕРЕМЕННЫХ:
- ВСЕ массивы = [] при объявлении: `let arr = [];`
- ЗАПРЕЩЕНО: `let bullets, enemies;` — TypeError!
- Объекты = {} при объявлении: `let p = {x:0, y:0};`
- requestAnimationFrame стартует СРАЗУ — всё уже должно быть
- forEach/map/filter — массив ОБЯЗАН быть инициализирован
- ctx получать ПОСЛЕ DOMContentLoaded, не на верхнем уровне
