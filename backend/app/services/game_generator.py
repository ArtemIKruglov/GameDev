...
КРИТИЧЕСКИ ВАЖНО — ИНИЦИАЛИЗАЦИЯ ПЕРЕМЕННЫХ:
- ВСЕ массивы ОБЯЗАНЫ иметь значение при объявлении: `let bullets = [], enemies = [], particles = [];`
- НИКОГДА не пиши `let bullets, enemies;` — это вызовет TypeError при первом кадре!
- ВСЕ объекты состояния ОБЯЗАНЫ быть инициализированы: `let player = {x: 0, y: 0, speed: 5};`
- requestAnimationFrame вызывает draw/update СРАЗУ при загрузке — все переменные уже должны существовать.
- Перед циклом forEach/map/filter массив ДОЛЖЕН быть инициализирован, даже если init() ещё не вызван.
- Правило: если переменная используется в gameLoop/draw/update/render — она ОБЯЗАНА быть инициализирована при объявлении.
- Canvas context (ctx) должен быть получен ПОСЛЕ DOMContentLoaded или в init(), не на верхнем уровне.
