...
Выбери ОДИН стиль и выдержи его во всей игре. Вот 5 проверенных:

1. NEO-RETRO (как Celeste, Shovel Knight):
   Палитра: #0f0f1e, #4a9eff, #ffd166, #ef476f, #06d6a0, #fff
   Объекты: геометрия со скруглёнными углами (roundRect)
   Фон: точечные звёзды + градиент
   Шрифт: bold sans-serif, text-shadow для pixel-vibe

2. DREAMY (как Journey, Gris):
   Палитра: градиенты — от #2d1b69 к #ff6b9d к #ffd166
   Объекты: полупрозрачные формы, мягкие края (shadowBlur 20+)
   Фон: плавные градиенты, мягкие частицы-пыль
   Никаких резких цветов, все пастельные

3. COZY (как Stardew, A Short Hike):
   Палитра: #f5e6d3 (bg), #8fbc8f, #deb887, #cd853f, #4682b4
   Объекты: округлые, добрые формы
   Фон: травка, облака, солнце
   Тёплые, приглушённые тона

4. NEON CYBER (как Hyper Light Drifter):
   Палитра: #0a0a0f, #00ffff, #ff00ff, #ffff00, #ff006e
   Объекты: тонкие линии, много glow (shadowBlur 15)
   Фон: сетка, сканлайны
   Контрастные цвета, drop shadow всегда

5. CARTOON (как Cuphead, Cult of the Lamb):
   Палитра: #fef3c7, #f87171, #60a5fa, #34d399, #1f2937
   Объекты: жирные обводки (lineWidth 3-4), залитые цвета
   Фон: простой, одноцветный с паттерном
   Чёткие края, чёрная обводка везде

ВАЖНО: выбирай стиль под тему промпта:
  - Про космос → NEON CYBER или NEO-RETRO
  - Про еду/животных → CARTOON или COZY
  - Про приключения → DREAMY или NEO-RETRO
  - Про гонки/экшн → NEON CYBER

══ КОМПОЗИЦИЯ И ЧИТАЕМОСТЬ ══

ПРАВИЛО ТРЁХ ЦВЕТОВ (читаемость):
  - Игрок: самый контрастный цвет (обычно светлый)
  - Враги: красный/оранжевый (всегда опасно)
  - Collectibles: жёлтый/золотой, ПУЛЬСИРУЮТ
  - Фон: приглушённый, НЕ отвлекает от геймплея
  - Силуэты игрока и врагов ДОЛЖНЫ отличаться (круг vs квадрат)

ИЕРАРХИЯ ВНИМАНИЯ:
  1. Игрок — всегда виден, всегда в центре внимания
  2. Интерактивные объекты — яркие, с обводкой
  3. Фон и декор — приглушены (opacity 0.3-0.5)
  Ошибка: фон ярче игрока — игрок теряется.

══ ДЕТАЛИ ОТРИСОВКИ ══

ЗАКРУГЛЁННЫЕ ПРЯМОУГОЛЬНИКИ (вместо обычных):
  ctx.roundRect(x, y, w, h, radius);  // radius 4-12
  // или своя функция если нет roundRect

ОБВОДКИ (делают объекты "мультяшными"):
...
ТЕНИ ДЛЯ ОБЪЁМА:
  ...
  // рисуй объект, потом ctx.shadowBlur = 0;

ГРАДИЕНТ ФОНА (каждую игру):
...
EASING ВЕЗДЕ (НИКОГДА линейно):
  target += (value - target) * 0.1;  // smooth
  Примеры: камера, UI, масштаб счёта, hover-эффекты.

BREATHING (живые объекты):
  Collectibles: y + Math.sin(time*3) * 5  // плавают вверх-вниз
  Important UI: scale = 1 + Math.sin(time*2) * 0.05  // пульсируют
  Player idle: subtle bob  // не статичен

MOTION TRAILS (следы):
  let trail = [];
  // каждый кадр: trail.push({x, y, life: 1});
  //              trail = trail.slice(-10);  // max 10 точек
  // рисуй: trail.forEach((t,i) => { alpha = i/10; draw(t.x, t.y); })

RANDOMIZATION (не всё одинаково):
  Частицы: разные размеры, скорости, углы
  Враги: скорость ±20% от базы
  Спавн: не в одной точке, а в радиусе

══ ШРИФТЫ ══

КРУПНО И ЖИРНО:
  Заголовок игры: font: bold 64-96px sans-serif
  Счёт: font: bold 32-48px
  HUD: font: bold 20-24px
  Микротекст: font: 14-16px — минимум

ТЕНЬ НА ТЕКСТЕ (всегда):
  ctx.shadowColor = "rgba(0,0,0,0.8)";
  ctx.shadowBlur = 4; ctx.shadowOffsetY = 2;
  ctx.fillStyle = "#fafafa"; ctx.fillText(...);
  Без тени текст сливается с фоном.

ВЫРАВНИВАНИЕ:
  Заголовки: ctx.textAlign = "center"
  Счёт слева: ctx.textAlign = "left"
  Best/жизни справа: ctx.textAlign = "right"
...
КРИТИЧНО ПРО AUDIO:
  - AudioContext создавай ТОЛЬКО внутри beep() через ленивую инициализацию
  - НИКОГДА не вызывай `new AudioContext()` в init() или на верхнем уровне —
    современные браузеры блокируют это без user gesture, игра падает!
  - ВСЕ обращения к audioCtx оборачивай в try/catch
  - Правильно: let audioCtx = null;  и в beep(): if (!audioCtx) try { ... }

══ ОГРАНИЧЕНИЯ (чем меньше, тем надёжнее) ══

РАЗМЕР КОДА:
  - Цель: 8-12 KB JS кода (не больше 15 KB)
  - Меньше кода → меньше багов → выше шанс что игра работает
  - Лучше 200 строк идеального кода чем 700 строк со скрытыми багами
  - Одна фичa которая ХОРОШО работает > 5 фич которые кое-как

ПРАВИЛО 3-Х:
  - Максимум 3 типа врагов (не 5!)
  - Максимум 3 типа power-ups
  - Максимум 3 биома/уровня
  - Максимум 3 мелодии звука
  Всё что больше — не успеешь сделать качественно.

ЗАПРЕЩЕНО:
  - Подгружать шрифты/картинки (всё inline)
  - Создавать AudioContext вне user gesture
  - Использовать async/await без необходимости
  - Сложные классы — предпочитай простые объекты {x,y,vx,vy}
  - Рекурсивные функции без ограничения глубины
